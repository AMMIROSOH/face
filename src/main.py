...
from process import capture, detection, candidates, recognition, gui
...
if __name__ == "__main__":
    ...
    size_frame = int(np.prod(FRAME_SHAPE) * np.dtype(np.uint8).itemsize)
    size_frame_gui = int(np.prod(FRAME_SHAPE_GUI) * np.dtype(np.uint8).itemsize)
    size_info = int(np.prod((LOC_LENGTH + CONF_LENGTH + LANDS_LENGTH) * DETECTION_LENGTH) * np.dtype(np.float32).itemsize)
    ...
    shm_cap_gui_frame = shared_memory.SharedMemory(create=True, size=size_frame_gui)
    ...
    # first global message indicates that system is running
    shm_global_msg.buf[0] = 1

    q1, q2, q3, q4 = Queue(), Queue(), Queue(), Queue()

    shms_capture = (shm_global_msg.name, shm_cap_frame.name, shm_cap_gui_frame)
    ...
    shms_fr = (shm_global_msg.name, shm_fr_frame.name, shm_fr_info.name, shm_gui_frame.name, shm_gui_info.name)
    shms_gui = (shm_global_msg.name, shm_gui_frame.name, shm_gui_info.name, shm_cap_gui_frame)

    p1 = Process(target=capture, args=(shms_capture, q1))
    p2 = Process(target=detection, args=(shms_fd, q1, q2))
    p3 = Process(target=candidates, args=(shms_post, q2, q3))
    p4 = Process(target=recognition, args=(shms_fr, q3, q4))
    ...
    shm_cap_gui_frame.close(); shm_cap_gui_frame.unlink()
